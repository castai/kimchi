...
// inside the 90-day session-start notice window regardless of when tests run.
...
describe("deprecated model notification", () => {
	...
	it("notifies when session starts with a deprecated model that has a replacement", async () => {
		...
		const models: ModelMetadata[] = [
			{
				...
				deprecated_at: isoWithinDays(30),
		...
		expect(ctx.ui.notify as Mock).toHaveBeenCalledWith(
			`Model "${deprecatedModelId}" is deprecated and will be retired on ${isoWithinDays(30).slice(0, 10)}. Switch to "${replacementModelId}" for better performance.`,
			"warning",
		)
	})
	...
	it("notifies with fallback message when deprecated model has no replacement", async () => {
		...
		const models: ModelMetadata[] = [
			{ slug: deprecatedModelId, display_name: "Kimi K2.6 Old", deprecated_at: isoWithinDays(30), ...modelProps },
			{ slug: "active-model", display_name: "Active Model", ...modelProps },
		]
		setupAvailableModels(models)
		...
		expect(ctx.ui.notify as Mock).toHaveBeenCalledWith(
			`Model "${deprecatedModelId}" is deprecated and will be retired on ${isoWithinDays(30).slice(0, 10)}. It may be removed in a future update.`,
			"warning",
		)
	})
	...
	it("warns once per deprecated model while cycling, even when cycling back and forth", async () => {
		...
		expect(cycleTwoCtx.ui.notify as Mock).toHaveBeenCalledWith(
			`Model "kimi-k2.5-old" is deprecated and will be retired on ${isoWithinDays(30).slice(0, 10)}. It may be removed in a future update.`,
			"warning",
		)
	})
	...
	it("does not warn when the retirement date is beyond the 90-day notice window", async () => {
		const modelProps: Omit<ModelMetadata, "slug" | "display_name" | "status" | "replacement"> = {
			provider: "kimchi-dev",
			reasoning: false,
			input_modalities: ["text"],
